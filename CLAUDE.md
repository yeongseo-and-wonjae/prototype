...
│   ├─ evidence.json           ★ 근거 카드 — 벡터에 들어가는 유일한 '왜'
│   ├─ sources.json            출처 레지스트리 (기관·연도·URL·이용조건). 검색 대상 아님
│   └─ seed.json               데모 환자 3명
├─ docker/
│   ├─ backend.Dockerfile
│   └─ frontend.Dockerfile
...
```python
collection.query(query_texts=[...], where={"$and": [
    {"surgery": "rotator_cuff"},
    {"phase_min": {"$lte": 1}}, {"phase_max": {"$gte": 1}},   # 여러 단계에 걸친 카드가 있다
    {"tear_track": {"$in": ["대·광범위", "공통"]}},            # 파열 크기로 근거가 갈린다
]}, n_results=5)
```
색인 대상: 공개 프로토콜 조각, 문헌 근거 카드. **운동 라이브러리와 금지 목록은 벡터에 넣지 않는다** (태그 필터·규칙 검사 대상).

**수치는 벡터에 넣지 않는다.** 보조기 기간·각도 상한·금지 목록은 `standard_protocol.json`과 `rules.py`가 들고 검사한다.
벡터에는 그 판정의 **근거(왜)**만 넣는다 — 검색이 놓쳐도 안전 규칙은 그대로 돌아야 하기 때문이다.

근거 카드(`data/evidence.json`)는 카드 하나에 주장 하나. **원문 문장을 복제하지 않고 수치·사실만 자체 표현으로 옮긴다**
(병원 프로토콜은 대부분 저작권 보유 문서다). 출처는 `source_id`로 `sources.json`을 가리키고, 원문 PDF는 저장소에 두지 않는다.
`tear_track`이 `소~중`/`대·광범위`로 갈리는 것이 가장 중요하다 — 문서 간 편차의 최대 원인이고,
대파열 환자에게 소~중에서만 성립하는 근거(조기 가동 안전)가 딸려가면 안 된다.
`needs_review: true`인 수치는 **화면에 "자문 검증 전"으로 표시하되 규칙에는 넣지 않는다.**
...
**진료 중에 쓰는 화면이다. 클릭을 줄이고 알아서 넘어가게 한다.**
- 환자를 열면 `st.switch_page()`로 다음 화면으로 바로 간다. 프로토콜이 이미 확정된 환자는 처방으로 직행한다
- 사진을 올리면 바로 추출한다. 처방 화면에 들어오면 초안을 알아서 만든다
- **같은 것을 두 번 보여주지 않는다.** 추천 목록과 담은 목록을 따로 두면 7개를 고르는 데 14개 블록을 스크롤해야 한다. 표 하나에서 체크로 고른다
- **판정과 적용 버튼은 표 위에 둔다.** 운동이 늘어도 결정 지점이 아래로 밀리지 않게
- 고르기는 `@st.fragment` 안에서 처리한다 — 한 번 누를 때마다 화면 전체가 다시 그려지지 않게
- 발송 후에는 다음 갈 곳(인박스 / 다음 환자)을 준다
- 지금 어디쯤인지 `ui.steps()`로 늘 보인다

**주의**: Streamlit은 위젯 변경 시 스크립트 전체가 재실행됨. AI 호출 결과는 반드시 `st.session_state`에 저장하고, 같은 환자에 대해 **한 번만** 호출할 것(`draft_for`).
