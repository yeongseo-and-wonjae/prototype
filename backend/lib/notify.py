...
def send_kakao(name: str | None, token: str, exercises: list[dict], today: date | None = None) -> dict:
    """실제 발송 대신 알림톡 미리보기를 돌려준다."""
    today = today or date.today()
    lines = [f"[리햅톡] {name or '환자'}님, 오늘의 재활 운동입니다",
             f"{today.month}월 {today.day}일 · 총 {len(exercises)}개 · 약 {sum(e.get('minutes', 5) for e in exercises)}분",
             ""]
